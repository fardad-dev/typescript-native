#!/usr/bin/env node
// tsn-compiler — the ts native compiler CLI.
//   tsn-compiler <file.ts> [-o out] [--emit-cpp]
...
program
  ...
  .option(
    "-o, --output <path>",
    "output executable path (defaults to the source basename)",
  )
  .option(
    "--emit-cpp",
    "also write the generated C++ source alongside the binary",
    false,
  )
