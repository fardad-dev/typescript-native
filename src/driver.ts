...
// Returns the C++ file path plus the extra clang link args the program needs
// (`-lcurl` when it uses `fetch`; `[]` otherwise — so a non-fetch link is unchanged).
function emitCppFile(opts: Options): { cppPath: string; linkArgs: string[] } {
  ...
  const { cpp, usesFetch } = emit(mod); // stage 3: IR -> C++ source
  ...
  return { cppPath, linkArgs: usesFetch ? ["-lcurl"] : [] };
}

export function compile(opts: Options): void {
  const { cppPath, linkArgs } = emitCppFile(opts);
  buildExecutable(cppPath, opts.output, linkArgs); // stage 4: clang++ -> executable
...
export async function compileAsync(opts: Options): Promise<void> {
  const { cppPath, linkArgs } = emitCppFile(opts);
  await buildExecutableAsync(cppPath, opts.output, linkArgs); // stage 4: clang++
  if (!opts.emitCpp) fs.unlinkSync(cppPath);
}
